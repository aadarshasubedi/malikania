...
JsonValue LoaderDirectory::requires(const JsonObject &object, const std::string &name, JsonType type) const
{
	...
	if (value.typeOf() != type) {
		throw std::runtime_error("invalid type given for property `" + name + "'");
	}

	return value;
}

JsonValue LoaderDirectory::optional(const JsonObject &object, const std::string &name, JsonType type, JsonValue def) const
{
	JsonValue value = object[name];
...
LoaderDirectory::LoaderDirectory(std::string path) noexcept
	: m_path(std::move(path))
{
}

GameSettings LoaderDirectory::gameSettings()
{
